...
# ─────────────────────────────────────────────
st.markdown("---")
st.markdown("© Africa AI Consulting Group").header-flex img {
    width: 80px;
}
...
<div class="header-flex">
    <img src="https://via.placeholder.com/80x80/C9A84C/000000?text=AI">
    <div>
        <h1 style="color:white;margin:0;">Dashboard Bancaire — Scoring Crédit & Churn</h1>
        <p style="color:#C9A84C;margin:0;">Africa IA Consulting · Fall Oumar</p>
    </div>
</div>
""", unsafe_allow_html=True)
...
def load_data():
    ...
    # Sauvegarde des colonnes texte pour filtres
    df_display = df.copy()
    ...
    # Prédictions
    df_display['Churn_Proba'] = model.predict_proba(X_scaled)[:, 1]

    df_display['Profil_Risque'] = pd.cut(
        df_display['Churn_Proba'],
        bins=[0, 0.3, 0.6, 1],
        labels=['🟢 Faible', '🟡 Modéré', '🔴 Élevé']
    )
    ...
    return df_display, model, auc, fpr, tpr, cm, importances


df, model, auc, fpr, tpr, cm, importances = load_data()
...
age_range = st.sidebar.slider("Âge", int(df['Age'].min()), int(df['Age'].max()), (18, 92))

risk_filter = st.sidebar.multiselect(
    "Profil risque",
    options=df['Profil_Risque'].unique(),
    default=df['Profil_Risque'].unique()
)
...
df_filtered = df[
    ...
    (df['Profil_Risque'].isin(risk_filter))
]
...
col1.metric("Clients", len(df_filtered))
col2.metric("Churn (%)", f"{df_filtered['Exited'].mean()*100:.1f}%")
col3.metric("Balance Moyenne", f"{df_filtered['Balance'].mean():.0f} €")

# ── GRAPHIQUES ─────────────────────────────────────────────
col1, col2 = st.columns(2)

with col1:
    geo = df_filtered.groupby('Geography')['Exited'].mean().reset_index()
    fig = px.bar(geo, x='Geography', y='Exited', title="Churn par pays")
    st.plotly_chart(fig, use_container_width=True)

with col2:
    fig2 = px.histogram(df_filtered, x='Age', nbins=30, title="Distribution âge")
    st.plotly_chart(fig2, use_container_width=True)
...
top = df_filtered.nlargest(20, 'Churn_Proba')
st.dataframe(top[['CustomerId','Age','Geography','Balance','Churn_Proba']])

# ── DOWNLOAD ───────────────────────────────────────────────
csv = top.to_csv(index=False).encode('utf-8')
st.download_button("⬇️ Télécharger clients à risque", csv, "clients_risque.csv")

# ── FOOTER ────────────────────────────────────────────────
st.markdown("---")
st.markdown("© Africa IA Consulting")
