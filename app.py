...
logo_tag = (
    f'<img src="data:image/jpeg;base64,{logo}" style="width:90px;height:90px;object-fit:contain;">'
    if logo else
    '<div style="width:90px;height:90px;background:linear-gradient(135deg,#1a4a8a,#0D2A4A);'
    'border-radius:50%;display:flex;align-items:center;justify-content:center;'
    'border:2px solid #C9A84C;font-size:28px;color:#C9A84C;font-weight:bold;">AI</div>'
)
...
.brand-name {{ font-family: 'Rajdhani', sans-serif; font-size: 28px; font-weight: 700; color: white; letter-spacing: 2px; line-height: 1.1; }}
.brand-sub {{ color: #C9A84C; font-size: 11px; letter-spacing: 3px; text-transform: uppercase; }}
.brand-tags {{ display: flex; gap: 12px; margin-top: 6px; font-size: 10px; color: #7a99bb; letter-spacing: 1px; }}
.divider-v {{ width: 1px; height: 70px; background: #C9A84C55; margin: 0 24px; }}
.header-title {{ font-family: 'Rajdhani', sans-serif; font-size: 34px; font-weight: 700; color: white; letter-spacing: 2px; line-height: 1.0; }}
.header-subtitle {{ color: #C9A84C; font-size: 15px; font-weight: 600; letter-spacing: 2px; }}
.header-tagline {{ color: #7a99bb; font-size: 11px; margin-top: 4px; letter-spacing: 1px; }}
.header-tagline span {{ color: #C9A84C; }}
.header-right {{ text-align: right; font-size: 12px; color: #7a99bb; border: 1px solid #1a3a5c; border-radius: 8px; padding: 10px 16px; background: rgba(255,255,255,0.03); }}
.header-right .date-label {{ font-size: 10px; color: #C9A84C; text-transform: uppercase; letter-spacing: 1px; }}
.live-dot {{ display: inline-block; width: 8px; height: 8px; background: #4CAF50; border-radius: 50%; margin-right: 5px; }}

/* ── KPI CARDS ── */
.kpi-card {{ background: #0D1B2A; border: 1px solid #1a3a5c; border-radius: 14px; padding: 18px 20px; display: flex; align-items: center; gap: 16px; min-height: 110px; position: relative; overflow: hidden; }}
...
.kpi-icon {{ font-size: 30px; flex-shrink: 0; width: 52px; height: 52px; border-radius: 12px; display: flex; align-items: center; justify-content: center; }}
...
.kpi-label {{ font-size: 11px; color: #7a99bb; text-transform: uppercase; letter-spacing: 1.5px; font-weight: 600; }}
.kpi-value {{ font-family: 'Rajdhani', sans-serif; font-size: 38px; font-weight: 700; color: white; line-height: 1.1; }}
.kpi-sub {{ font-size: 11px; color: #4a6a8a; margin-top: 2px; }}

/* ── SECTION ── */
.section {{ background: #0D1B2A; border: 1px solid #1a3a5c; border-radius: 12px; padding: 16px; margin-bottom: 14px; }}
.section-title {{ font-size: 13px; font-weight: 600; color: #C9A84C; margin-bottom: 8px; display: flex; align-items: center; gap: 8px; }}

/* ── SIDEBAR ── */
[data-testid="stSidebar"] {{ background: #071421 !important; border-right: 1px solid #1a3a5c !important; }}
[data-testid="stSidebar"] .block-container {{ padding: 1rem !important; }}
.sidebar-section {{ background: #0D1B2A; border: 1px solid #1a3a5c; border-radius: 12px; padding: 14px; margin-bottom: 12px; }}
.sidebar-title {{ font-family: 'Rajdhani', sans-serif; font-size: 16px; font-weight: 700; letter-spacing: 2px; color: white; margin-bottom: 14px; display: flex; align-items: center; gap: 8px; }}
.sidebar-label {{ font-size: 12px; color: #7a99bb; margin-bottom: 4px; display: flex; align-items: center; gap: 6px; }}
.count-box {{ background: #0D1B2A; border: 1px solid #1a3a5c; border-radius: 12px; padding: 16px; text-align: center; margin-bottom: 12px; }}
.count-value {{ font-family: 'Rajdhani', sans-serif; font-size: 32px; font-weight: 700; color: #C9A84C; }}
.count-label {{ font-size: 12px; color: #7a99bb; }}
.export-btn {{ background: linear-gradient(135deg, #0D2A4A, #1a3a5c); border: 1px solid #2196F3; border-radius: 10px; padding: 14px; text-align: center; cursor: pointer; color: white; font-size: 13px; font-weight: 600; }}

/* ── INSIGHT CARDS ── */
.insight-card {{ background: rgba(255,255,255,0.03); border: 1px solid #1a3a5c; border-radius: 10px; padding: 14px; margin-bottom: 10px; display: flex; gap: 12px; align-items: flex-start; }}
.insight-icon {{ font-size: 22px; flex-shrink: 0; width: 40px; height: 40px; border-radius: 10px; display: flex; align-items: center; justify-content: center; }}
.insight-text {{ font-size: 12px; color: #a0b8cc; line-height: 1.5; }}
.insight-text span {{ color: #C9A84C; font-weight: 600; }}

/* ── Streamlit overrides ── */
.stMultiSelect > div {{ background: #0D1B2A !important; border-color: #1a3a5c !important; }}
...
def load():
    if os.path.exists("churn.csv"):
        ...
        df = pd.DataFrame({
            ...
            'CustomerId': np.random.randint(int(1e7), int(9e7), n),
            'Surname': ['Client'] * n,
...
with st.sidebar:
    ...
    st.download_button("", csv, "clients_selection.csv", "text/csv", key="dl",
                       help="Télécharger", use_container_width=True)
...
# ───────────────────────── TABS ─────────────────────────
st.markdown("""
<div class="tab-bar" style="display:flex;gap:8px;margin:18px 0 14px 0;">
    <div class="tab-item tab-active" style="padding:10px 22px;border-radius:10px;border:1px solid #2196F3;
         background:linear-gradient(135deg,#1a4a8a,#0D2A4A);color:white;font-size:13px;font-weight:500;">
        📊 Vue Générale
    </div>
    <div class="tab-item" style="padding:10px 22px;border-radius:10px;border:1px solid #1a3a5c;
         color:#7a99bb;font-size:13px;font-weight:500;">🤖 Modèle ML</div>
    <div class="tab-item" style="padding:10px 22px;border-radius:10px;border:1px solid #1a3a5c;
         color:#7a99bb;font-size:13px;font-weight:500;">🏷️ Scoring Crédit</div>
    <div class="tab-item" style="padding:10px 22px;border-radius:10px;border:1px solid #1a3a5c;
         color:#7a99bb;font-size:13px;font-weight:500;">⚠️ Clients à Risque</div>
</div>
""", unsafe_allow_html=True)
...
with g3:
    fig3 = go.Figure(go.Histogram(x=df_f['Age'], nbinsx=20, marker_color=BLUE, opacity=0.85))
...
with g4:
    ...
    fig4.update_layout(yaxis_ticksuffix='%', showlegend=False, height=260, xaxis_title="Nombre de produits")
...
with r2:
    ...
    fig_cm.update_layout(height=300, xaxis_title="Prédit", yaxis_title="Réel", xaxis=dict(side='top'))
...
with r3:
    ...
    fig_fi = go.Figure(go.Bar(
        ...
        orientation='h', marker_color=GOLD, opacity=0.9
    ))
    style_fig(fig_fi, "")
...
with r4:
    ...
    <div class="insight-card">
        ...
        <div class="insight-text">
            <span>AUC = {auc:.3f}</span> — le modèle identifie correctement {auc*100:.1f}% des clients qui vont partir.
        </div>
    </div>
    <div class="insight-card">
...
# ───────────────────────── TABLE CLIENTS À RISQUE ─────────────────────────
st.markdown('<div class="section-title" style="margin-top:18px;">⚠️ Top 20 Clients à Risque</div>', unsafe_allow_html=True)
st.dataframe(
    df_f.nlargest(20, 'Proba')[['CustomerId', 'Age', 'Geography', 'Gender',
                                  'CreditScore', 'Balance', 'Proba', 'Risque']],
